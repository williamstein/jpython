...
var builtin_modules = {
  crypto: {
    createHash: function create_hash() {
      ...
      ans.digest = function hex_digest() {
        return ans.getHash("HEX");
      };
      return ans;
    },
  },
  ...
  fs: {
    readFileSync: function readfile(name) {
      if (
        ...
        namespace.virtual_file_system.read_file_sync
      ) {
    ...
    writeFileSync: function writefile(name, data) {
      if (
        namespace.virtual_file_system &&
        namespace.virtual_file_system.write_file_sync
      ) {
