...
(function () {
  ...
  function show_completions(completions) {
    var comps = document.getElementById("completions"),
      e = comps.lastChild;
    e.innerHTML = "";
    var groups = [],
      current_group = [];
    ...
    groups.forEach(function (group) {
      ...
      var longest_length =
        group.concat().sort(function (a, b) {
          return b.length - a.length;
        })[0].length + 1;
      group.forEach(function (word) {
        var node = document.createTextNode(
          word + "\xa0".repeat(longest_length - word.length)
        );
  ...
  function check_for_completions() {
    ...
    if (ss === undefined || input.selectionEnd !== ss)
      return hide_completions();
    var before = source.substr(0, ss);
    var ret = web_repl.find_completions(before);
    if (!ret || ret.length != 2) return hide_completions();
    var completions = ret[0],
      prefix = longest_common_prefix(completions),
      last_tok = ret[1];
    if (prefix == last_tok && completions.length == 1)
      return hide_completions();
  ...
  function on_load() {
    ...
    add_output(
      "RapydScript-ng " +
        RapydScript.rs_version +
        " (at commit: " +
        RapydScript.rs_commit_sha.substr(0, 10) +
        ")\n" +
        "Type RapydScript code into the box at the bottom and click the" +
        " Run button. You can press Tab for completions.",
      "blue"
    );
    ...
    document.getElementById("input").addEventListener("keyup", function () {
      setTimeout(update_completions, 0);
    });
    document
      .getElementById("completions")
      .firstChild.addEventListener("click", hide_completions);
  }
