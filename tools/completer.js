...
module.exports = function (compiler, options) {
  ...
  function object_names(obj, prefix) {
    ...
    function prefix_filter(name) {
      return prefix_len ? name.substr(0, prefix_len) === prefix : true;
    }
  ...
  function find_completions(line, ctx) {
    ...
    if (
      last_tok.value === "." ||
      (last_tok.type === "name" && compiler.IDENTIFIER_PAT.test(last_tok.value))
    ) {
      ...
      if (tokens.length > 1 && tokens[tokens.length - 2].value === ".") {
        ...
        var prefix = "",
          result;
        tokens.slice(0, tokens.length - 2).forEach(function (tok) {
          prefix += tok.value;
        });
        if (prefix) {
          try {
