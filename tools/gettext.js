...
function Gettext(catalog, filename) {
  this._visit = function (node, cont) {
    if (
      node instanceof RapydScript.AST_Call &&
      node.args &&
      node.args.length &&
      node.expression instanceof RapydScript.AST_Symbol
    ) {
          ...
          if (!(node.args[i] instanceof RapydScript.AST_String)) {
            console.error(
              "Translation function: " +
                name +
                " does not have a string literal argument at line: " +
                line +
                " of " +
                filename
            );
...
function write_output(catalog, options, write) {
  write =
    write ||
    function (x) {
      process.stdout.write(new Buffer(x, "utf-8"));
    };
  function print() {
  ...
  if (!options.omit_header) {
    ...
    header_line(
      "Project-Id-Version:",
      esc(options.package_name),
      esc(options.package_version)
    );
  ...
  function read_files(src) {
    src.forEach(function (f) {
      if (fs.lstatSync(f).isDirectory()) {
        var children = [];
        fs.readdirSync(f).forEach(function (x) {
          children.push(path.join(f, x));
        });
        read_files(children);
      } else files.push(f);
    });
