...
function ρσ_list_insert(index, val) {
  ...
  (ρσ_expr_temp = this)[
    typeof index === "number" && index < 0 ? ρσ_expr_temp.length + index : index
  ] = val;
}
...
ρσ_dict.prototype.update = (function () {
  var ρσ_anonfunc = function () {
    ...
    if (Array.isArray(iterable)) {
      for (var i = 0; i < iterable.length; i++) {
        m.set(
          iterable[typeof i === "number" && i < 0 ? iterable.length + i : i][0],
          iterable[typeof i === "number" && i < 0 ? iterable.length + i : i][1]
        );
      }
  ...
  for (var c = 1; c < arguments.length; c++) {
    ...
    while (p && p !== Object.prototype) {
      ...
      for (var i = 0; i < props.length; i++) {
        ...
        if (
          seen[typeof name === "number" && name < 0 ? seen.length + name : name]
        ) {
          continue;
        }
        seen[
...
function zip() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    iterators[typeof i === "number" && i < 0 ? iterators.length + i : i] = iter(
      arguments[typeof i === "number" && i < 0 ? arguments.length + i : i]
    );
  }
  ...
  if (as_array) {
    ...
    for (var k = 0; k < keys.length; k++) {
      ...
      ans.push(
        JSON.stringify(key) +
          ":" +
          ρσ_repr(x[typeof key === "number" && key < 0 ? x.length + key : key])
      );
    }
  }
...
define_str_func = (function () {
  var ρσ_anonfunc = function (name, func) {
    ...
    ρσ_str[typeof name === "number" && name < 0 ? ρσ_str.length + name : name] =
    ...
    var ρσ_anonfunc = function () {
          ...
          (function () {
            var ρσ_anonfunc = function (match, key) {
              ...
              return (
                "" +
                kwargs[
                  typeof key === "number" && key < 0 ? kwargs.length + key : key
                ]
              );
            };
            if (!ρσ_anonfunc.__argnames__)
      ...
      function apply_formatting(value, format_spec) {
        ...
        if (fill && value.length < width) {
          if (align === "<") {
            ...
            if (ρσ_in(value[0], "+- ")) {
              value =
                value[0] + repeat(fill, width - value.length) + value.slice(1);
            } else {
              value = repeat(fill, width - value.length) + value;
            }
          } else {
      ...
      function render_markup(markup) {
        ...
        if (lkey) {
          ...
          object =
            args[typeof idx === "number" && idx < 0 ? args.length + idx : idx];
          idx += 1;
        }
        ...
        if (ends_with_equal) {
          ans =
            "" + ρσ_str.format("{}", key) + "=" + ρσ_str.format("{}", ans) + "";
        }
        return ans;
      }
...
define_str_func(
  ...
  (function () {
    var ρσ_anonfunc = function (width, fill) {
      ...
      fill = fill || " ";
      return (
        new Array(left + 1).join(fill) + this + new Array(right + 1).join(fill)
      );
    };
    if (!ρσ_anonfunc.__argnames__)
      Object.defineProperties(ρσ_anonfunc, {
...
define_str_func(
  ...
  (function () {
    var ρσ_anonfunc = function (chars) {
      ...
      while (
        chars.indexOf(
          string[typeof pos === "number" && pos < 0 ? string.length + pos : pos]
        ) !== -1
      ) {
        pos += 1;
      }
